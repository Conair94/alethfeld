...
\]
where $\ket{\psi_\rho}_{AB}$ is the canonical purification $\ket{\psi_\rho} = (\sqrt{\rho} \otimes I)\ket{\Omega}$ with maximally entangled state $\ket{\Omega} = \frac{1}{\sqrt{d}}\sum_{i=1}^d \ket{i}_A\ket{i}_B$, and $\EU$ denotes the expectation over the Haar measure on the unitary group $U(d)$.
\end{theorem}
...
\begin{proofsteps}
    ...
    \item \textbf{Canonical Purification.} Define $\ket{\psi_\rho} = (\sqrt{\rho} \otimes I)\ket{\Omega}$ where $\ket{\Omega} = \frac{1}{\sqrt{d}}\sum_{i=1}^d \ket{i}_A\ket{i}_B$. \justify{Construction; A1, A2}
    \begin{proofsteps}
        \item Let $\{\ket{i}\}_{i=1}^d$ be an orthonormal basis for $\HA \cong \HB$.
        \item Define the maximally entangled state $\ket{\Omega} = \frac{1}{\sqrt{d}}\sum_{i=1}^d \ket{i}_A\ket{i}_B$.
